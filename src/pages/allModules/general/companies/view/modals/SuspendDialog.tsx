...
import { toggleCompanyIsActive } from 'src/store/apps/general/company'
...
const SuspendDialog = (props: Props) => {
  ...
  const onSubmit = () => {
    ...
    dispatch(toggleCompanyIsActive({ data: { id: data.id, isActive: data.isActive }, onSubmitSuccess, onSubmitError }))
  }
    ...
    <>
      <Dialog fullWidth open={open} onClose={handleClose} sx={{ '& .MuiPaper-root': { width: '100%', maxWidth: 512 } }}>
            ...
            {data.isActive === 1 ? (
              <>
                <Typography sx={{ color: 'error.main' }}>
                  Suspend Company {data.name} of type : {data.companyType.name}
                </Typography>
                <Typography>All users in this Company won't be able to login!</Typography>
              </>
            ) : (
              <Typography sx={{ color: 'error.main' }}>Activate Company {data.name} </Typography>
            )}
          </Box>
        </DialogContent>
        <DialogActions
        ...
        >
          ...
          >
            {data.isActive === 1 ? <span> Yes, Suspend company!</span> : <span> Yes, Activate company!</span>}
          </LoadingButton>
